...
import type { Dialect, KernelPass, PassRunner, PipelineDef } from "../kernel/index.ts";
...
/** A plugin pass may be definition-only or executable with an attached runner. */
export type PluginPassContribution =
  | Omit<KernelPass, "id">
  | {
      readonly pass: Omit<KernelPass, "id">;
...
export interface PluginContributions {
  ...
  /** Plugin-contributed dialects. */
  readonly dialects: readonly Dialect[];
  /** Plugin-contributed passes (id generated by registry). */
  readonly passes: readonly PluginPassContribution[];
  /** Plugin-contributed named pass pipelines. */
...
export interface TargetContribution {
  ...
  /** Optional named pipeline to run for this target during generation. */
